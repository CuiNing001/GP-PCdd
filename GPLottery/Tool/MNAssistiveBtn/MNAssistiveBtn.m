...
- (void)changePostion:(UIPanGestureRecognizer *)pan{
    ...
    if (pan.state == UIGestureRecognizerStateBegan) {
        ...
        if (frame.origin.x < screenW/2) {
            ...
            isOver = YES;
            // x大于1/2屏幕，返回右侧屏幕
        }else if (frame.origin.x > screenW/2){
            frame.origin.x = screenW-button.frame.size.width;
            isOver = YES;
        }
        ...
        if (frame.origin.y < 64) {
            ...
            isOver = YES;
            //y大于底部tabbar高度
        } else if (frame.origin.y+frame.size.height > screenH-64) {
            frame.origin.y = screenH - frame.size.height-64;
            isOver = YES;
        }
